...
- **Próximo passo imediato:** aluno baixa `checkpoint_best.pt` das runs do Kaggle (Swin-T V3 prioritário; ResNet/ViT opcionais) → coloca em `experiments/runs/<run>/` → roda `python -m src.interpretability.generate_maps --run-dir <run>`. Analisar se os mapas atendem regiões clinicamente plausíveis (hipocampo/ventrículos). Reservado: 2.5D (ADR-0011) e re-rodar ResNet V2 c/ persistência p/ McNemar das 3 arqs.
