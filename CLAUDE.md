...
- **Próximo passo imediato:** rodar treino real (não-smoke) do baseline ResNet-50 no Colab — `python -m src.training.run --epochs 20` em ambiente com GPU. Salvar resultados em `experiments/runs/`. Em paralelo: começar implementação dos modelos ViT-Base/16 e Swin-T em `src/models/`.
