...
- **V3-transformer executado (2026-05-23, F-0019):** regularização forte (ADR-0013) **destravou ganho**. **Swin-T V3 é o modelo campeão** (test bal_acc 0,616; macro_f1 0,551; AUC 0,836; best epoch 4; F1 minoritária 0,426). ViT V3 melhorou signif. sobre V2 (0,521→0,568, McNemar p≈0) mas segue overfittando (best ep2, 86M params demais p/ 242 pacientes). Swin V3 >> ViT V3 (McNemar p≈0). Overfit atenuado (val_loss pico 3,0→1,5). Teto ~0,62 (val pico 0,694).
- **Interpretabilidade implementada (2026-05-23, ADR-0014):** `src/interpretability/` com Attention Rollout (ViT, via hook em attn_drop) + Grad-CAM (ResNet `layer4` / Swin `layers[-1]` com reshape de tokens) + `generate_maps.py` (CLI). Validado end-to-end local com checkpoint dummy (gera grid + overlays). Roda em CPU local — não precisa Kaggle. Figuras versionadas em `results/interpretability/`.
- **Ablação de capacidade executada (2026-05-24, F-0022):** ResNet-18 (11,2M params) treinada — **pior de todas** (bal_acc 0,501 < ResNet-50 0,587; McNemar p=2,8e-6). **Hipótese "menos parâmetros generaliza melhor" REFUTADA**: confirma que overfit não era o gargalo da ResNet-50; o limite é a representação (fatia 2D única), não capacidade. Análise de arquiteturas FECHADA — 4 modelos comparados via McNemar. Ranking bal_acc: Swin-T V3 (0,616) > ResNet-50 V2 (0,587) > ViT-Base V3 (0,568) > ResNet-18 (0,501).
- **Próximo passo imediato:** todos os experimentos de modelo/arquitetura/capacidade fechados; interpretabilidade dos 3 principais feita. Alavanca restante = **2.5D** (ADR-0011, dados) — opcional. Gargalo real agora é **ESCRITA do TCC** (entrega da cópia ~2026-06-03): montar texto a partir de docs/ (22 Findings + 14 ADRs + FAQ + resultados consolidados).
...
|---|---:|---:|---:|---:|---:|---|
| ResNet-50 V1 | 0,699 | 0,623* | 0,554 | 0,804 | 1 | *instável (epoch 1, pré-colapso) |
| ResNet-50 V2 | 0,681 | 0,587 | 0,513 | 0,804 | 11 | baseline oficial CNN |
| ViT-Base/16 V2 | 0,730 | 0,521 | 0,483 | 0,822 | 2 | overfit severo |
| ViT-Base/16 V3 | 0,692 | 0,568 | 0,514 | 0,811 | 2 | V3>V2 signif. (McNemar p≈0) |
| Swin-T V2 | 0,723 | 0,594 | 0,528 | 0,823 | 2 | overfit (val pico 0,668) |
| **Swin-T V3** | 0,720 | **0,616** | **0,551** | **0,836** | 4 | **MELHOR MODELO** (F-0019) |
| ResNet-18 | 0,666 | 0,501 | 0,458 | 0,777 | 9 | ablação capacidade: pior de todas (F-0022) |
