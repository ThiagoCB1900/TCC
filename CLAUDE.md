...
- **Próximo passo imediato:** aluno roda V2 no Kaggle — `python -m src.training.run --run-name resnet50_v2_class3_weighted_kaggle` (defaults já são V2) + ablação `--no-class-weights`. Comparar bal_acc V1 (0,623) com V2 (esperado 0,68-0,75). Comparativo automático na última célula do notebook 03 mostra V1 vs V2 lado a lado.
